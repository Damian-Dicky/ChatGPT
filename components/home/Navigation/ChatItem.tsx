...
export default function ChatItem({ item, selected, onSelected }: Props) {
    ...
    return (
        <li
            ...
            key={item.id}
            className={`relative group flex items-center p-3 space-x-3 cursor-pointer rounded-md hover:bg-gray-800 font-sm ${
        ...
        >
            ...
            {editing ? (
                <input
                    autoFocus={true}
                    className="flex-1 min-w-0 bg-cyan-900 outiline-none"
                    value={title}
                    onChange={(e) => {
